...
function calcular_antiguedad_heladera(fechaInauguracion) {
    ...
    const diferenciaEnDiasDecimal = diferenciaEnMilisegundos / (1000 * 3600 * 24);
    const diferenciaEnDias = Math.floor(diferenciaEnDiasDecimal);
    ...
    else if(diferenciaEnDias <= 31 && diferenciaEnDias > 1) {
    ...
    if (!loggedInUser) {
        ...
        buttons.forEach(button => {
            button.addEventListener('click', function(event) {
                if (button.id !== 'create_account_button' && button.id !== 'login_button') {
                    event.preventDefault();
                }
            });
        });
...
document.addEventListener('DOMContentLoaded', () => {
    const loggedInUser = JSON.parse(localStorage.getItem('loggedInUser'));
    if (loggedInUser) {
        enviarSolicitudSuscripcionExistente(loggedInUser);
    }
});

function enviarSolicitudSuscripcionExistente(data) {
    fetch('http://heladerassolidarias.myvnc.com:4567/solicitudSuscripcionExistenteUsuario', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(respuesta => {
        if (respuesta === 'Colaborador Suscripto') {
            const loggedInUser = JSON.parse(localStorage.getItem('loggedInUser'));
            loggedInUser.suscripcion = 'suscripto';
            localStorage.setItem('loggedInUser', JSON.stringify(loggedInUser));
        }
    })
    .catch((error) => {
