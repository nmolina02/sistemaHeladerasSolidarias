...
@Entity
@DiscriminatorValue("VIANDAS_POR_COLABORADOR")
@javax.persistence.Table(name = "reporte_viandas_por_colaborador")
@Getter
public class ReporteViandasPorColaborador extends Reporte {
    @OneToMany(mappedBy = "reporte", cascade = CascadeType.ALL)
    public List<ViandasPorColaborador> viandasPorColaborador;

    @Transient
    public TipoReporte tipoReporte;

    public ReporteViandasPorColaborador(){
        super();
        this.viandasPorColaborador = GeneradorDeReportes.getInstance().getViandasPorColaborador();
        this.tipoReporte = TipoReporte.VIANDAS_COLABORADOR;
        for (ViandasPorColaborador viandas : viandasPorColaborador) {
            viandas.setReporte(this);
        }
    }
    ...
    public void generarReporteExcel() throws IOException {
        ...
        String fileName = "Viandas Por Colaborador (" + fechaInicialEncabezado + " - " + fechaFinalEncabezado + ").xlsx";
        String filePath = directoryPath.resolve(fileName).toString();

        this.setPathArchivo(filePath);

        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Viandas Por Colaborador (" + fechaInicialEncabezado + " - " + fechaFinalEncabezado + ")");

        Row headerRow = sheet.createRow(0);
        String[] columnHeaders = {"ID", "Colaborador", "Cantidad de viandas donadas"};
    ...
    public void convertirExcelAPdf(String excelFilePath, String pdfFilePath) throws IOException {
        ...
        String fechaInicial = this.getFechaInicial().format(this.getFormatter());
        String fechaFinal = this.getFechaFinal().format(this.getFormatter());
        String mainTitle = "Reporte de Viandas por Colaborador";
