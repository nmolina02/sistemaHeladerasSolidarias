...
@Entity
@DiscriminatorValue("MOVIMIENTOS_HELADERA")
@javax.persistence.Table(name = "reporte_movimientos_heladera")
@Getter
public class ReporteMovimientosHeladera extends Reporte {
    @OneToMany(mappedBy = "reporte", cascade = CascadeType.ALL)
    public List<MovimientosHeladera> movimientosPorHeladera;

    @Transient
    public TipoReporte tipoReporte;

    public ReporteMovimientosHeladera(){
        super();
        this.movimientosPorHeladera = GeneradorDeReportes.getInstance().getMovimientosPorHeladera();
        this.tipoReporte = TipoReporte.MOVIMIENTOS_HELADERA;
        for (MovimientosHeladera movimiento : movimientosPorHeladera) {
            movimiento.setReporte(this);
        }
    }
    ...
    public void generarReporteExcel() throws IOException {
        ...
        String fileName = "Movimientos Heladera (" + fechaInicialEncabezado + " - " + fechaFinalEncabezado + ").xlsx";
        String filePath = directoryPath.resolve(fileName).toString();

        this.setPathArchivo(filePath);

        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Movimientos Heladera (" + fechaInicialEncabezado + " - " + fechaFinalEncabezado + ")");

        Row headerRow = sheet.createRow(0);
        String[] columnHeaders = {"ID", "Heladera", "Cantidad de viandas ingresadas", "Cantidad de viandas retiradas"};
    ...
    public void convertirExcelAPdf(String excelFilePath, String pdfFilePath) throws IOException {
        ...
        String fechaInicial = this.getFechaInicial().format(this.getFormatter());
        String fechaFinal = this.getFechaFinal().format(this.getFormatter());
        String mainTitle = "Reporte de Movimientos Heladera";
